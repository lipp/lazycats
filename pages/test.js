...
const threshold = [...Array(thresholdSteps + 1).keys()].map(x => x / thresholdSteps)
      ...
      <div data-xyz='ddd'>
        <Observer options={{threshold}} id='foo'>
          ...
          <Box color='white' opacity={1}>
            <Observable onIntersectionChange={({intersectionRatio}) => this.setState({xx: intersectionRatio})}>
              <h1>{this.state.xx}</h1>
            </Observable>
          </Box>
        </Observer>
